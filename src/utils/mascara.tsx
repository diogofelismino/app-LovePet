...
export function aplicarMascaraDateTime(value: any) {
  ...
  if (maskedValue.length <= 2) {
    ...
    return maskedValue.replace(/(\d{2})(\d{1,2})/, '$1/$2'); // Dia/Mês (DD/MM)7
    ...
    return maskedValue.replace(/(\d{2})(\d{2})(\d{1,4})(\d{1,2})/, '$1/$2/$3 $4');
  } else {
    return maskedValue.replace(/(\d{2})(\d{2})(\d{4})(\d{1,2})(\d{1,2})?/, '$1/$2/$3 $4:$5'); // Dia/Mês/Ano Hora:Minuto (DD/MM/YYYY HH:mm)
  }
};